...
A tiny, stateful Gopherspace “overlay”:
...
Make sure `ghc` is installed (e.g., `sudo apt install ghc`).
Needs `curl` in PATH (used to fetch remote gopher menus/pages)

Then, configure it to work with a Gopher daemon, like with
[Venusia](https://github.com/someodd/venusia)'s `routes.toml`:
...
Location-only keys; usernames never appear in path keys.
...
Commands to be used by users (in a room):
    ...
    pher host[:port]/sel – “teleport” (set last room and render there)
...
We begin the code by importing everything!
